...
export function MobileNavigation() {
  ...
  return (
    <nav aria-label="Navigazione mobile" className="glass fixed inset-x-0 bottom-0 z-50 grid grid-cols-6 border-t border-[var(--line)] pb-[env(safe-area-inset-bottom)] lg:hidden">
      {items.map((item) => {
        const active = isActive(pathname, item.href);
        const Icon = item.icon;
        return (
