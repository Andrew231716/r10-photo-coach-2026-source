...
import { Check, Copy, Database, LogIn, LogOut, ShieldCheck } from "lucide-react";
  ...
  if (account) return (
    <div className="premium-panel p-6 sm:p-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div><p className="font-mono text-xs uppercase tracking-[0.16em] text-[var(--cyan)]">Profilo sincronizzato</p><h2 className="font-display mt-2 text-3xl font-semibold">{account.displayName}</h2></div>
      ...
      <button type="button" onClick={signOut} disabled={busy} className="mt-7 inline-flex min-h-11 items-center gap-2 border border-[var(--line)] px-4 text-sm text-[var(--muted)] hover:text-[var(--paper)]"><LogOut size={16} /> Esci dal profilo</button>
    </div>
  );
